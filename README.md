...
- **Unified tagging:** Traces, metrics, and OTLP logs all use the **same full** OTLP **resource** (service.*, process.*, telemetry SDK, OS, deployment, optional K8s/cloud, `env`, `version`, `ddsource`, etc.). Using the same resource for all three providers is required because the OTel SDK's `WithResource` always merges `resource.Environment()` (i.e. raw `OTEL_RESOURCE_ATTRIBUTES` / `OTEL_SERVICE_NAME`) into the final resource; our values must be present so they override any bad env values (e.g. unexpanded ACA placeholders). **Each slog log line** adds a flat **`service`** string (Service facet) plus **high-signal** resource fields only—`env`, `version`, `deployment.environment.name`, `ddsource`, `datadog.log.source`, cloud region, K8s/ACA/container tags from env, and anything from **`OTEL_RESOURCE_ATTRIBUTES`** (non-host). **`service.*`**, **`process.*`**, **`telemetry.*`**, **`os.*`**, and deprecated **`deployment.environment`** are **not** repeated on every slog line (avoids argv noise and SDK clutter); those stay on the OTLP resource for correlation. **`trace_id`** / **`span_id`** are added when the log context has an active span ([semantic mapping](https://docs.datadoghq.com/opentelemetry/mapping/semantic_mapping/)).
